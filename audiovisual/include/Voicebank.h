...
namespace soundmath
{
	...
	// N ins, N outs
	template <typename T, int N> class Voicebank : public Multichannel<T, N>
	{
	private:
	...
	public:
		...
		Voicebank(int order, const std::vector<std::complex<T>>& radii)
		{
			setup(order, radii);
		}
		...
		void modulate(const std::vector<std::complex<T>>& radii)
		{
			for (int i = 0; i < N; i++)
